...
The recommended way of passing credential options to your Anypoint Platform CLI installation is using the `config.json` file located inside the `macOS: ~/Library/Preferences/@mulesoft/anypoint-cli-nodejs
Unix: ~/.config/@mulesoft/anypoint-cli-nodejs
Windows: %LOCALAPPDATA%\@mulesoft\anypoint-cli-nodejs` directory, which is not automatically created during installation. 
