...
|xref:anypoint-cli::api-mgr.adoc#api-mgr-alert-add-legacy[api-mgr:alert:add-legacy] | Creates an API instance alert
|xref:anypoint-cli::api-mgr.adoc#api-mgr-alert-list[api-mgr:alert:list] | Lists alerts for an API instance
|xref:anypoint-cli::api-mgr.adoc#api-mgr-alert-list-legacy[api-mgr:alert:list-legacy] | Lists alerts for an API instance from the legacy alert
...
Creates an API instance alert with the name passed in `name` for the API instance ID passed in `<apiInstanceId>` using the new alert monitoring system.

This command introduces a more detailed way to configure alert notifications and conditions. Alerts are enabled by default when created and can be paused later.
...
This command is functionally identical to the previous `api-mgr:alert:add` command. Use it to manage alerts that are still monitored by the legacy system.
...
Lists alerts for the API instance passed in `<apiInstanceId>` using the new alert monitoring system.

The output reflects the new alert object structure. The **Enabled** status is derived from the alert's current state (for example, a paused state means the alert is not enabled).
...
This command is functionally identical to the previous `api-mgr:alert:list` command. The output reflects the structure of the legacy alert objects, including the `enabled` boolean property and the original recipient format.
