...
| `--enabled` | Sets whether the alert should be enabled or not. Include the flag to enable it.
...
| `--policyId <num>` | ID of a policy applied to the API instance to trigger `policy-violation` alert type.
...
|===
|Flag |Description |Example
|`--createdDate <value>` | Date the app was created, not the contract | `--sort createdDate <apiInstanceId>`
|`--id <value>` | ID of the app | `--sort id <apiInstanceId>`
|`--limit <value>` | Number of results to retrieve | `--limit 2`
|`--name <value>` | Name of the app | `--sort name <apiInstanceId>`
...
|`--sort <value>` | Sorts the contracts based on the supported values, supported values are `createdDate`, `id`, and `name`. | `--sort id` |===
