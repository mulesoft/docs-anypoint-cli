...
NOTE: Flex Gateway API instances created using the Anypoint CLI do not support multiple upstream services. To create Flex Gateway API instances with multiple upstream services by using the API Manager UI, see xref:api-manager::create-instance-task-flex.adoc[].
