...
| `--durationUnit <val>` | Condition occurrence period duration unit. +
...
| `--policyInstanceId=<num>` | ID of a policy applied to API instance to trigger `response-code` alert type.
...
This command deletes the API instance passed in `<apiInstanceId>`. If the API instance is deployed, this command first undeploys the API instance before deleting.
...
Besides the default `--help` flag and standrad flags, this command also accepts:
...
This command accepts only the default flag `--help`s.
