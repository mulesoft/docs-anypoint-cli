...
Besides the default `--help` and `-o`/`--output` options, this command also has a `gatewayVersion` option to specify the gateway version you want to download.
...
Besides the default `--help` and `-o`/`--output` options, this command also has the `-m, --muleVersion4OrAbove` option which has `true` or `false` values. +
