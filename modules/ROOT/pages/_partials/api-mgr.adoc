...
Besides the default `--help` and `-o`/`--output` options, this command also accepts a `gatewayVersion` option to specify the gateway version you want to download.
...
When the `[apiInstanceId]` option is specified, this command lists the policies applied to that API instance.

Besides the default `--help` and `-o`/`--output` options, this command also accepts the `-m, --muleVersion4OrAbove` option which has `true` or `false` values. +
