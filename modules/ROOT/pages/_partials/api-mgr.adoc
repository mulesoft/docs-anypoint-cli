...
|xref:anypoint-cli::api-mgr.adoc#api-mgr-application-add-owner[api-mgr:application:add-owner] | Adds an owner to a client application
|xref:anypoint-cli::api-mgr.adoc#api-mgr-application-import[api-mgr:application:import] | Imports a client application from an external client identity provider
|xref:anypoint-cli::api-mgr.adoc#api-mgr-application-possible-owners-list[api-mgr:application:possible-owners-list] | Lists possible owners for a client application in the current organization
|xref:anypoint-cli::api-mgr.adoc#api-mgr-client-provider-list-importable[api-mgr:client-provider:list-importable] | Lists client identity providers with import enabled in the current organization
...
This command downloads the API proxy ZIP file of the API instance passed in `<apiInstanceId>` to a local directory specified in `<targetPath>`. You cannot download the API proxy of an Omni Gateway API instance.
...
Edits the API instance passed in `<apiInstanceId>`. Editing a deployed Omni Gateway API instance redeploys the instance.
...
|Flag |Description |Example
| `-f, --isFlex`
| Indicates whether this is an Omni Gateway API instance. +
...
| `--serviceName`
| WSDL service name +
Omni Gateway does not support this flag | `--serviceName ExampleServerName`

| `--serviceNamespace`
| WSDL service namespace. Omni Gateway does not support this flag. |`--serviceNamespace exampleServiceName`
...
| `--output` | Specifies the response format, supported values are `table` (default) and `json` |  `--output json`
| `--sort` | Sorts the results in the field name passed +
Supported values are: `id`, `name`, `createdDate`, and `updatedDate` | `--sort updatedDate` 
|===
...
[NOTE] 
====
Omni Gateway API instances created with Anypoint CLI do not support multiple upstream services. To create Omni Gateway API instances with multiple upstream services, see: 
...
| `--deploymentType`
| Deployment type +
Options: `cloudhub2`, `cloudhub`, `hybrid`, `rtf` (required) | `--deploymentType hybrid`   
...
Adds the owner passed in `<id>` with the entity type passed in `<entityType>` to the client application passed in `<applicationId>`.
...
Imports the client application passed in `<clientId>` from the external client identity provider passed in `<providerId>`. The client identity provider must have the import feature enabled.
...
|===
|Flag |Description |Example
| `--description` | Description of the client application to import. | `--description "Example application description"`
| `--url` | URL of the client application to import. | `--url https://example.com`
|===
...
|===
|Flag |Description |Example
| `--search` | Search string to filter results. | `--search example`
| `--limit` | Number of results to retrieve. | `--limit 10`
| `--offset` | Number of results to skip. | `--offset 10`
|===
...
Lists the client identity providers that have the import feature enabled in the current organization.
...
|`--offset` | Offsets the amount of APIs passed | `--offset 3`
| `--output` | Specifies the response format, supported values are `table` (default) and `json` |  `--output json`
|`--sort` | Sorts the contracts by the criteria associated with their client applications +
...
NOTE: If you are udpating an included Mule Gateway policy, you can include only the changed values in the configuration.
However, if you are updating a custom policy, you must send the full configuration each time a custom policy is edited.
