...
Besides the default `--help` and `-o` options, this command also accepts a `gatewayVersion` option to specify the gateway version you want to download.
...
Besides the default `--help` and `-o` options, this command also accepts the `-m, --muleVersion4OrAbove` option which has `true` or `false` values. +
