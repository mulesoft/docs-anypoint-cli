...
|`clustered`| Enables clustered nodes, which require at least two replicas. Default:.`disabled` +
...
----
Updates the settings of an existing application specified by `<appID>`. 
To get the `<appID>`, run the `runtime-mgr application list` command.
...
|`updateStrategy`| Updates the strategy used. +
Default: `rolling`. | `recreate`
|`clustered`| Enables clustered nodes, which require at least two replicas. +
Default: `disabled`. +
Supported values: `true`, `false`. |`false` 
|`lastMileSecurity`| Enables Last Mile Security. +
Default: `disabled`. +
