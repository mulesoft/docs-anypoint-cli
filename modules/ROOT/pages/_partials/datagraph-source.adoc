...
|`--auth-custom` a|Use if you authenticate to your API by passing custom header parameters and values, such as query params or headers.
{sp} +
{sp} +
Required for query params:

* `--query-params: <comma-separated list of key-value pairs, for example, key1:value1,key2:value2>`
+
Or
...
* `--headers: <comma-separated list of key-value pairs, for example, key1:value1,key2:value2>`
+
Or
...
|`--cert` | Specify the client certificate to use for mutual transport layer security (mTLS).
