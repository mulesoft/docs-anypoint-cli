...
|--criteria <criteria>
|The `criteria` option The `criteria` option enables you to apply filters to select the list of APIs to which the profile rulesets will apply. The option is followed by a comma-separated list of identifiers for tags, categories, and scope as follows: `tag:tag1,tag:tag2,category:catname:cat1,scope:apitype`
