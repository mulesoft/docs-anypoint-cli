...
`> governance:api:inspect [flags] <api-definition>`
...
`> governance:api:validate [flags] <api-definition>`
...
* To use an existing `exchange.json` file that defines your API project's ruleset dependencies, ensure that the `exchange.json` file is included in the folder or ZIP file that you specify in `api-definition`. If the `exchange.json` file is present, the command downloads all the ruleset dependencies and validates against those rulesets. The ruleset dependencies are present in the `exchange.json` file only if dependencies are defined for that API project in API Designer. See xref:api-governance::find-conformance-issues.adoc#add-rulesets[Add Rulesets to Your Project].
...
|Local ruleset definitions. The `rulesets` flag is followed by a comma-separated list of ruleset YAML files. 
|`--remote-rulesets <ruleset-asset-identifier>,<ruleset-asset-identifier>,...`
|Remote ruleset definitions. The `remote-rulesets` flag is followed by a comma-separated list of ruleset asset identifiers. An asset identifier is a group ID, asset ID, and version (GAV) that uniquely identifies each asset in Exchange. For example: `<group_id>/<asset_id>/<version>,<group_id>/<asset_id>/<version>`
...
* The following example command validates using the rulesets in the `exchange.json` file in the zipped folder `order-api-1.0.0-raml.zip` in the `/MyApis/` directory path. It validates the APIs found in the zipped folder `order-api-1.0.0-raml.zip` in the `/MyApis/` directory path.
+
[source,copy]
----
...
----
+
* The following example command validates using the rulesets and APIs found in the folder `order-api-1.0.0-raml` in the `/MyApis/` directory path.
+
[source,copy]
----
...
----
+
* The following example command validates using the rulesets listed with the `--rulesets` flag and any listed in the `exchange.json` file in the zipped folder named `order-api-1.0.0-raml.zip`. It validates the APIs found in the zipped folder  `order-api-1.0.0-raml.zip` in the `/MyApis/` directory path.
+
[source,copy]
----
...
----
+
* The following example command validates using the remote ruleset specified using the ruleset identifier (GAV) and any rulesets listed in the `exchange.json` file in the zipped folder named `order-api-1.0.0-raml.zip`. It validates the APIs found in the zipped folder `order-api-1.0.0-raml.zip` in the `/MyApis/` directory path.
+
[source,copy]
----
...
----
+
* The following example command validates using the remote ruleset specified using the ruleset identifier (GAV) and any rulesets listed in the `exchange.json` file in the zipped folder named `order-api-1.0.0-raml.zip` in the `~/Downloads/` directory path. It validates the remote API specified using the API identifier (GAV).
+
[source,copy]
----
...
`> governance:ruleset:info [flags] <governance-ruleset>`
