...
|Enables you to apply filters against which to evaluate to identify rulesets. Available filters include `category`, `tag`, `status` (lifecycle state), `env-type` (production or sandbox), `env-id` (environment ID), and `scope` (the targeted API types, `rest-api` and `async-api`). If `env-type` or `env-id` is used, only APIs that have instances are filtered. If `env-type` or `env-id` is used, only APIs that have instances are filtered. Example: tag:tag1,category:category1:value,category:category2:value2,status:stable,status:development,status:deprecated,scope:rest-api,scope:async-api,env-type:production
|===
...
See <<exchange-asset-identifiers>>.
|--criteria <filtertype:filtervalue>,...
|Enables you to apply filters to select the list of APIs to which the profile rulesets will apply. Specify a list of comma-separated filters where each filter has a type and value in the format filterType:filterValue. Available filters include `category`, `tag`, `status` (lifecycle state), `env-type` (production or sandbox), `env-id` (environment ID), and `scope` (the targeted API types, `rest-api` and `async-api`). If `env-type` or `env-id` is used, only APIs that have instances are filtered. Example: tag:tag1,category:category1:value,category:category2:value2,status:stable,status:development,status:deprecated,scope:rest-api,scope:async-api,env-type:production
