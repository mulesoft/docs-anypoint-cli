...
* An asset identifier, which is a group ID, asset ID, and version (GAV) that uniquely identifies an API project in Exchange. This requires the `--remote` option. 
...
|Remote ruleset definitions. The `remote-rulesets` option is followed by a comma-separated list of ruleset asset identifiers, each of which is the group ID, asset ID, and version (GAV) that uniquely identifies each ruleset in Exchange. For example: `<group_id>/<asset_id>/<version>,<group_id>/<asset_id>/<version>`

See <<exchange-asset-identifiers>>.
|--remote 
|Flag to indicate that the validation should be done against a published API. The value passed in `api-definition` is the asset identifier, which is the group ID, asset ID, and version (GAV) that uniquely identifies the API in Exchange. For example: `<group_id>/<asset_id>/<version>
...
|The `ruleset-gavs` option is a list with the asset identifier for each ruleset, formatted as follows: `<group_id>/<asset_id>/<version>,<group_id>/<asset_id>/<version>`. An asset identifier is a unique group ID, asset ID, and version (GAV) that identifies each ruleset in Exchange.
...
|--remote
|Indicates that the ruleset to clone is published in Exchange and that the `ruleset` parameter is the asset identifier for the ruleset. The asset identifier is the group ID, asset ID, and version (GAV) that uniquely identifies the ruleset in Exchange. For example: `<group_id>/<asset_id>/<version>
