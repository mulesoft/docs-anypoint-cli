...
`> governance ruleset validate [options] <ruleset-zip-file> | <ruleset-folder> | <exchange-json-file> | <ruleset-yaml-file>`

This command validates the definition format of specified rulesets. You can specify the path to the ruleset definition YAML file (`ruleset-yaml-file`), ZIP file (`ruleset-zip-file`), or folder (`ruleset-folder`). If you specify a folder, you can optionally specify additional parameters for the `exchange.json` file (`exchange-json-file`) and a ruleset YAML file (`ruleset-yaml-file`) after the folder name to more specifically identify the rulesets you want to validate.
