...
<3> The constraint is the ruleset to which this set of issues applies 
<4> Severity level is shown for each issue
<5> The message describes the issue
<6> The target is the node in AMF 
<7> The range is the beginning line number and offset and the end line number and offset in the API definition where the issue occurs
<8> The location is the file in which the issue occurs; it could be the main file or any of its dependencies
