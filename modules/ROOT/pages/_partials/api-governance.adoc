...
+
Filter by `category` or `tag` fields only if your APIs in Exchange have tags or categories.
+
* `status`: Lifecycle state of development or stable
...
+
If `env-type` or `env-id` is used, only APIs that have instances are filtered. APIs in the development lifecycle do not have instances. 
+
* `scope`: Targeted API types, `rest-api` or `async-api` 
