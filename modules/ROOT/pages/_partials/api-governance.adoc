...
The API definition passed in `api-definition` can be an API project ZIP file or folder. The rulesets against which to validate are determined as follows:

* If you want to validate directly against rulesets published in Exchange, use the `--remote-rulesets` option. 

* If you want to validate against local rulesets, use the `--rulesets` option.

* If the API project includes an `exchange.json` file that defines dependencies, the command downloads all dependencies and validates against the rulesets in the project. 
...
|The `rulesets` option is followed by a comma-separated list of ruleset YAML files. If a duplicate ruleset is included in the project dependencies and the `--rulesets` option list, that ruleset will be validated twice.
|--remote-rulesets <ruleset-asset-identifiers>
|The `remote-rulesets` option is followed by `ruleset-asset-identifiers`, a list with the group ID, asset ID, and version (GAV), which is the unique asset identifiers for each ruleset. Use a comma separated list formatted as follows: `<group_id>/<asset_id>/<version>,<group_id>/<asset_id>/<version>`. See <<ruleset-asset-identifiers>>.
...
anypoint-cli governance api validate --remote-rulesets xxx  ~/Downloads/order-api-1.0.0-raml.zip
----
