...
`> governance ruleset clone [options] <ruleset_gavs> <new_title> <new_description>` 
...
The `ruleset-gavs` parameter is followed by comma-separated list with the group ID, asset ID, and version (GAV) for each ruleset, formatted as follows: `<group_id>/<asset_id>/<version>,<group_id>/<asset_id>/<version>` 
...
[source,copy]
----
anypoint-cli governance ruleset clone xxx
