...
|xref:anypoint-cli::api-governance.adoc#governance-ruleset-classes[governance:ruleset:classes] | Lists targetable classes, optionally filtered by domain
|xref:anypoint-cli::api-governance.adoc#governance-ruleset-domains[governance:ruleset:domains] | Lists available metadata domains
...
|xref:anypoint-cli::api-governance.adoc#governance-ruleset-properties[governance:ruleset:properties] | Lists properties and types for a target class
|xref:anypoint-cli::api-governance.adoc#governance-ruleset-resolve[governance:ruleset:resolve] | Resolves a user-facing alias to its canonical target class and property path
|xref:anypoint-cli::api-governance.adoc#governance-ruleset-simplify[governance:ruleset:simplify] | Flattens nested paths and removes redundant logical wrappers in a ruleset YAML file
...
|xref:anypoint-cli::api-governance.adoc#governance-ruleset-version[governance:ruleset:version] | Shows the ruleset authoring tooling version
|===
...
Lists the classes that can be used as targets in a governance ruleset, optionally filtered by domain.
...
`> governance:ruleset:completions [flags] <file>`

Returns context-aware completions at a cursor position in the ruleset YAML file specified in `file`.
...
[source,copy]
----
anypoint-cli-v4 governance:ruleset:completions myruleset.yaml
----
...
`> governance:ruleset:domains [flags]`

Lists the available metadata domains that can be targeted when authoring governance rulesets. Available domains include `amf`, `mcp`, `api-instance`, `api-project`, `governance`, and `community`.
...
Returns the context and instructions for generating a governance ruleset from the natural-language description provided in `intent`.
...
[source,copy]
----
anypoint-cli-v4 governance:ruleset:generate "ensure all API operations have a summary and at least one response"
----
...
`> governance:ruleset:properties [flags] <class>`

Lists the properties and their types for the class specified in `class`. Use this information when defining property paths in a ruleset rule. To see available classes, run the `governance:ruleset:classes` command.
...
[source,copy]
----
anypoint-cli-v4 governance:ruleset:properties Operation
----
...
Resolves the user-facing alias specified in `term` to its canonical target class and property path.
...
[source,copy]
----
anypoint-cli-v4 governance:ruleset:resolve operation-summary
----
...
`> governance:ruleset:simplify [flags] <file>`

Flattens nested property paths and removes redundant logical wrappers from the ruleset YAML file specified in `file`. The simplified ruleset is written to standard output.
...
[source,copy]
----
anypoint-cli-v4 governance:ruleset:simplify myruleset.yaml > myruleset-simplified.yaml
----
...
`> governance:ruleset:validate-authoring [flags] <file>`

Validates the ruleset YAML file specified in `file` against the authoring model, checking for correct target classes, valid property paths, compatible constraint types, and valid severity values.
...
[source,copy]
----
anypoint-cli-v4 governance:ruleset:validate-authoring myruleset.yaml
----
...
`> governance:ruleset:version [flags]`

Shows the version of the ruleset authoring tooling bundled with the governance plugin.
