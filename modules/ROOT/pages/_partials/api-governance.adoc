...
|--remote 
|Flag to indicate that the validatation should be done against a published API. The value passed in `api-definition` is the asset identifier, which is the group ID, asset ID, and version (GAV) that uniquely identifies the API in Exchange. For example: `<group_id>/<asset_id>/<version>
