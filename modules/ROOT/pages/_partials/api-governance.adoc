...
Commands that require access to Anypoint Platform require adequate permissions to execute the functionality they are trying to execute.

The `governance` commands have the following restrictions:
...
+
* The same as the API Governance UI:
+
Any commands that require log in to Anypoint Platform to execute successfully, such as `profile create`, `profile update`, `profile delete`, and commands that use `--remote` flags
