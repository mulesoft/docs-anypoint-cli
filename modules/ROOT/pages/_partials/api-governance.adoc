...
|`--api=<api>`
|The API project against which you want to evaluate. The command uses the criteria in the project's `exchange.json` file. | `--api order-api-1.0.0-raml.zip` +
...
For example: `--anypoint-cli-v4 governance:api:validate /MyApis/order-api-1.0.0-raml.zip`
...
|Local ruleset definitions. The `rulesets` flag is followed by a list of ruleset YAML files separated by spaces. | `--rulesets /MyRulesets/ruleset1.yaml /MyRulesets/ruleset2.yaml`
...
See <<exchange-asset-identifiers>>. | `--remote-rulesets 68ef9520-24e9-4cf2-b2f5-620025690913/open-api-best-practices/1.0.1`
|`--remote`
|Flag to indicate that the validation should be done against a published API. The value passed in `api-specification` is the API's asset identifier.
+
