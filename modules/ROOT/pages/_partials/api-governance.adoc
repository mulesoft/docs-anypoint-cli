...
`> governance api evaluate [options] <api-project-zip>`

This command evaluates the API project and uses the filters such as API type, tags, and categories in the `exchange.json` file to determine the rulesets that would be applied to this API for those filters. This gives developers a way to determine the rulesets that their APIs pass or fail for the same filters a governance administrator might set in an API Governance profile. 
...
|A list of filters separated by commas, where each filter has a type and value in the format filterType:filterValue. Available filters include category, tag, and scope (the targeted API types). Example: tag:tag1,category:category1:value,category:category2:value2,scope:async-api,scope:rest-api
|===
...
[source,copy]
----
anypoint-cli governance api evaluate --criteria=tag:best,category:API Type:Experience API,scope:rest-api ~/Downloads/order-api-1.0.0-raml.zip
----
...
[source,copy]
----
anypoint-cli governance init --types=restapi,asyncapi --name=my-ruleset --output=~/temp/myruleset.yaml
