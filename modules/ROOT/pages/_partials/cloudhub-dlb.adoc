...
This command has exactly the same characteristics as the <<cloudhub load-balancer whitelist add>> command.

[NOTE]
We favor the use of the term "allowlist" instead of "whitelist" in adherence to our Inclusive Product Language Program.

== cloudhub load-balancer allowlist remove

----
> cloudhub load-balancer allowlist remove [options] <name> <cidrBlock>
----

This command has exactly the same characteristics as the <<cloudhub load-balancer whitelist remove>> command.
