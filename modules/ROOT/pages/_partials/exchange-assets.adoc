...
[NOTE]
When using `--output json`, the `versions` property in the response includes information about all available versions of the asset, not only the version specified in `<assetIdentifier>`. This is expected behavior.
