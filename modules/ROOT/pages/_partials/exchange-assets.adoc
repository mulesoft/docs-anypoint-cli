...
Argument `assetIdentifier` should be formatted as follows: `([group_id]/)<asset_id>/<version>`. +
If `group_id` is not specified, it defaults to the currently selected Organization ID. +
Besides the default `--help` and `-o`/`--output` options, this command also accepts the `--name` option to set a new asset page name.
...
Besides the default `--help` and `-o`/`--output` options, this command also accepts the `--draft` option to list non-published resources in the asset portal.
