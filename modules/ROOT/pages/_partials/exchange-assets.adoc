...
| `--files` | Asset file, identified as `classifier.packaging` or `packaging`, and its file path, for example, `--files='{"oas.zip":"​/-api.raml"}'`

To send multiple files, use the same option multiple times:

`--files='{"oas.zip": "my-demo-oas.zip"}' --files='{"custom.zip": "~/my-custom-file.zip"}'` | `exchange:asset:uploadv2 --files'{"pom.xml": "directory/pom-file.xml"}' --files='{"raml.raml": "./my-api.raml"}'`
| `--type` | Asset type
