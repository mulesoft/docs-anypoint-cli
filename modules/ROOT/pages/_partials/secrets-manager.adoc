...
|xref:anypoint-cli::secrets-manager.adoc#secret-TLS-flex-create[secrets-mgr:tls-context:flex-gateway:create]| Creates a new Omni Gateway TLS context secret
|xref:anypoint-cli::secrets-manager.adoc#secret-TLS-flex-describe[secrets-mgr:tls-context:flex-gateway:describe]| Shows details of a Omni Gateway TLS context secret
|xref:anypoint-cli::secrets-manager.adoc#secret-TLS-flex-list[secrets-mgr:tls-context:flex-gateway:list]| Lists all Omni Gateway TLS context secrets in a secret group
|xref:anypoint-cli::secrets-manager.adoc#secret-TLS-flex-modify[secrets-mgr:tls-context:flex-gateway:modify]| Modifies a Omni Gateway TLS context secret
|xref:anypoint-cli::secrets-manager.adoc#secret-TLS-flex-replace[secrets-mgr:tls-context:flex-gatway:replace]| Replaces an existing Omni Gateway TLS context secret
...
|`--key-passphrase`|Passphrase required for `JKS`, `PKCS12` and `JCEKS` keystore secrets +
Optional for `PEM` keystore secrets| `--key-passphrase examplePassphrase`
...
Creates a new Omni Gateway TLS context secret in the secret group specified by `--group-id`, and using the name specified by `--name`
...
Returns the details of an Omni Gateway TLS context secret specified by `--id` from the secret group specified by `--group-id`
...
Modifies the name or expiration date for an Omni Gateway TLS context secret specified by `--id` from the group specified by `--group-id` 
...
Replaces an existing Omni Gateway TLS context secret specified by `--id`, from the secret group specified by `--group-id`, using the type specified by `--type`
